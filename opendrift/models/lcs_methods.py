...
class LocalEquirectangular():
    """Local equirectangular (flat-earth) transform, in meters relative to (lon0,lat0).

    Simple trigonometric alternative to a projected CRS, so that a seeding grid with
    constant spacing in meters can be built from a lon/lat reader without the user
    having to supply a projected CRS.

    Exposes the same call signature as pyproj.Proj, i.e. proj(lon,lat) -> (x,y) in
    meters and proj(x,y,inverse=True) -> (lon,lat), so both can be used interchangeably.

    Spacing is exact along y, and exact along x at lat0 (distortion grows as
    cos(lat)/cos(lat0) away from lat0), which is fine for typical LCS domains.
    """
    ...
    def __init__(self, lon0, lat0):
        self.lon0 = lon0
        self.lat0 = lat0
        self.m_per_deg_lat = np.pi * self.R / 180.
        self.m_per_deg_lon = self.m_per_deg_lat * np.cos(np.radians(lat0))

    def __call__(self, x, y, inverse=False):
        if inverse:  # (x,y) meters -> (lon,lat)
            return (self.lon0 + np.asarray(x) / self.m_per_deg_lon,
                    self.lat0 + np.asarray(y) / self.m_per_deg_lat)
        # (lon,lat) -> (x,y) meters
        return ((np.asarray(x) - self.lon0) * self.m_per_deg_lon,
                (np.asarray(y) - self.lat0) * self.m_per_deg_lat)


def seeding_grid(self, reader=None, delta=None, domain=None):
    """Build the cartesian frame and seeding positions used for LCS computations.

    The Cauchy-Green tensor is computed by finite differences over a grid with
    constant spacing <delta> (meters), so we need a cartesian frame plus a way to
    project particle positions back into it.

    - reader already in a projected/cartesian CRS : used as-is, domain in reader coords.
    - reader in lon/lat (WGS84) : build a local equirectangular frame centred on the
      domain, which turns <delta> into a lon/lat step by simple trigonometry, so no
      projected CRS has to be supplied. Domain is then expected in lon,lat.

    Parameters: see calculate_green_cauchy_tensor()

    Returns:
        reader : the reader actually used (resolved if none was given)
        proj : callable with the pyproj.Proj signature, (lon,lat) <-> (x,y) in meters
        xs, ys : 1D cartesian coordinates of the seeding frame (meters)
        X, Y : 2D meshgrid of xs, ys
        lons, lats : 2D seeding positions, in WGS84
    """
    if reader is None:
    ...
    if proj.crs.to_epsg() == 4326:
        # reader is natively in lon,lat : build a local cartesian frame by trigonometry
        # so that the seeding grid has a constant spacing of <delta> meters
        if xmax > 360:
            raise ValueError('reader is in lon,lat so domain %s must be given as '
                             '[lon_min,lon_max,lat_min,lat_max]' % (domain,))
        lon_min, lon_max, lat_min, lat_max = xmin, xmax, ymin, ymax
        proj = LocalEquirectangular(lon0=0.5 * (lon_min + lon_max), lat0=0.5 * (lat_min + lat_max))
        xmin, ymin = proj(lon_min, lat_min)
        xmax, ymax = proj(lon_max, lat_max)
        logger.info('reader native coordinate system is WGS84, using a local equirectangular '
                    'frame centred on (%s,%s) to define the LCS seeding frame' % (proj.lon0, proj.lat0))
        logger.info('converting LCS domain from WGS84 (%s) to cartesian (%s)'
                    % ([lon_min, lon_max, lat_min, lat_max], [xmin, xmax, ymin, ymax]))

    xs = np.arange(xmin, xmax, delta)
    ys = np.arange(ymin, ymax, delta)
    logger.debug('seeding frame for LCS computation (cartesian) [xmin=%s,xmax=%s,ymin=%s,ymax=%s]'
                 % (xs[0], xs[-1], ys[0], ys[-1]))
    X, Y = np.meshgrid(xs, ys)
    lons, lats = proj(X, Y, inverse=True)

    return reader, proj, xs, ys, X, Y, lons, lats
...
def calculate_green_cauchy_tensor(self,
    ...
    domain (list): [xmin, xmax, ymin, ymax] defining the spatial domain for seeding (same coords system as <reader>,
        i.e. lon/lat for a lon/lat reader). If None, the domain is automatically determined based on the reader's extent.
    ...
    # define seeding frame
    reader, proj, xs, ys, X, Y, lons, lats = seeding_grid(
        self, reader=reader, delta=delta, domain=domain)
